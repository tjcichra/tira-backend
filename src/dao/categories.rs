...
pub async fn archive_category_by_id(conn: &TiraDbConn, category_id: i64) -> QueryResult<usize> {
    ...
    conn.run(move |c| diesel::update(categories.filter(id.eq(category_id))).set(archived.eq(true)).execute(c))
        .await
}

/// DAO function for creating a category.
pub async fn create_category(conn: &TiraDbConn, category: CreateCategory, creator_id_parameter: i64) -> QueryResult<usize> {
    use crate::schema::categories::dsl::*;

    conn.run(move |c| {
        diesel::insert_into(categories)
            .values((category, creator_id.eq(creator_id_parameter), created.eq(Utc::now().naive_utc())))
            .execute(c)
    })
    .await
}
...
pub async fn get_categories(conn: &TiraDbConn, filter_archived: Option<bool>) -> QueryResult<Vec<Category>> {
    use crate::schema::categories::dsl::*;

    match filter_archived {
        Some(filter_archived) => conn.run(move |c| categories.filter(archived.eq(filter_archived)).load(c)).await,
